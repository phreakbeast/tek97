...
bool tek_sb_init(TekSpritebatch* sb, u32 width, u32 height)
{
    ...
    //Shader
    sb->shader  = Shader::load("sprite.glsl");
    if (!sb->shader)
	return false;
    ...
    GLCall(glBufferData(GL_ARRAY_BUFFER, SB_BUFFER_SIZE, nullptr, GL_STATIC_DRAW));

    int loc_pos = sb->shader->get_attrib_loc("a_position");
    int loc_uv = sb->shader->get_attrib_loc("a_uv");
    int loc_tid = sb->shader->get_attrib_loc("a_tid");
    int loc_color = sb->shader->get_attrib_loc("a_color");
...
void tek_sb_flush(TekSpritebatch* sb)
{
    ...
    sb->shader->uniform_mat4("u_mvp", &sb->ortho, 1);


    sb->shader->uniform_int("u_texture_0", 0);
    sb->shader->uniform_int("u_texture_1", 1);
    sb->shader->uniform_int("u_texture_2", 2);
    sb->shader->uniform_int("u_texture_3", 3);
    sb->shader->uniform_int("u_texture_4", 4);
    sb->shader->uniform_int("u_texture_5", 5);
    sb->shader->uniform_int("u_texture_6", 6);
    sb->shader->uniform_int("u_texture_7", 7);
